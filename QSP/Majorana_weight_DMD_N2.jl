...
function heisenberg_weight_dynamics(
    ...
    generators, coeffs = extract_hamiltonian_coeffs_and_ops(H)
...
function fit_dmd(X::AbstractMatrix{<:Real}; r::Union{Nothing,Int}=nothing, tol=1e-10)
    ...
    A_tilde = Matrix(Ur' * X2 * Vr * Diagonal(1.0 ./ sr))

    eig = eigen(A_tilde)
    λ = eig.values
    W = eig.vectors

    Φ = Matrix(X2 * Vr * Diagonal(1.0 ./ sr) * W)
    b = Φ \ complex.(X[:, 1])
...
function delay_embed(snapshots::Vector{<:AbstractVector}, q::Int)
    ...
    @assert q >= 1
    @assert m >= q + 1 "Need at least q+1 snapshots for DMD."
...
embed_snapshots(snapshots; q::Int=1) =
    q == 1 ? hcat(snapshots...) : delay_embed(snapshots, q)
