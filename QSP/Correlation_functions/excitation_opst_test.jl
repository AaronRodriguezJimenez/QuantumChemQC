...
function single_excitation_pool(n_system::Int)
    N = n_system    
    pool = Vector{PauliBasis{N}}([])
...
function doubles_excitation_pool(n_system::Int)
    N = n_system    
    pool = Vector{PauliBasis{N}}([])
...
singles = single_excitation_pool(4)
for p in singles
    display(p)
end
println("Total operators in single excitation pool: ", length(singles))

doubles = doubles_excitation_pool(4)
for p in doubles
    display(p)
end
println("Total operators in double excitation pool: ", length(doubles))
