...
"""
 Here we perform the calculation for H Chains, using the precomputed RHF MO integrals in the sto-3g basis, 
    and test the DBF algorithm for finding the ground state energy.

        Ground state FCI energy: -5.157978039118 
"""
function run()
    # Get precomputed active space spinorbitals tensors
    data_path = "/Users/admin/PycharmProjects/pyQCTools/QSP/small_molecules/HChains/h10-RHF_tensors.npz" 
...
function dbf_gstate(H::PauliSum{N, T}) where {N,T}
    ...
    max_rots_per_grad=1 
    ket, occ, kidx  = string_to_ket("11111111110000000000") #Leading gst singlet configuration H10
