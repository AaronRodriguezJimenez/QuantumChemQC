#= 
  Taken from Kyle Sherbert's ADAPT code, with modifications.
...
function fermi_a(q,n)
    ...
    return QCUtils.kron_concat(ops)
end
...
function molecular_hamiltonian(mol::Molecule; m=nothing)
    ...
    if m !== nothing
        H = QCUtils.extendoperator(H, n, m)
    end
...
function measure_energy(H::Hermitian, ψ::Vector{ComplexF64})
    ...
    if length(ψ) > N
        ...
        ψ = QCUtils.qubitspace(ψ, n)          # PROJECTED STATEVECTOR
        ψ ./= norm(ψ)                       # RE-NORMALIZED
    end
    return real(QCUtils.expectation(H,ψ))     # ENERGY CALCULATION
end
